...
Texture Story::story_texture;

bool Story::init(vec2 screen) {
	story_texture.load_from_file(textures_path("game intro.png"));
	float w = screen.x; // story_texture.width;
	float h = screen.y; // story_texture.height;
	float wr = w * 0.5f;
	float hr = h * 0.5f;
