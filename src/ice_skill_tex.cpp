...
bool Iceskilltex::init(vec2 screen, int skill_num)
{
	switch (skill_num) {
		case 1:
			ice_texture.load_from_file(ice_skill1());
...
void Iceskilltex::update_ice(bool paused, float degree)
{
	if (paused) {
		if (degree == 0.f) {
			get_texture(0);
...
void Iceskilltex::get_texture(int loc)
{
	float sw = 120.f;
	float w = 720.f;
...
vec2 Iceskilltex::get_position()const
{
	return m_position;
}

void Iceskilltex::light_up() {
	m_light_up = 1;
}

void Iceskilltex::blue_up() {
	m_light_up = 0;
}